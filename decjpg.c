...
void main()
{
	...
	unsigned char *mydata[DATASIZE];
	AVFormatContext *ofmt_ctx = NULL;//其包含码流参数较多，是一个贯穿始终的数据结构，很多函数都要用到它作为参数
	const char *out_filename = "out.yuv";//输出文件路径，在这里也可以将mkv改成别的ffmpeg支持的格式，如mp4，flv，avi之类的
	int ret;//返回标志
	...
	AVStream *out_stream = add_vidio_stream(ofmt_ctx, CODEC_ID_CYUV);//创造输出视频流
